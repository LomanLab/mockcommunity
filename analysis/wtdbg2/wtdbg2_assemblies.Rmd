...
```{r}
assemblies = assemblies %>% mutate(Label=paste(Platform,Limit,Overlaps))
ggplot(assemblies,aes(x=Label, y=ContigSize, fill=Taxon)) + geom_bar(stat="identity") + coord_flip() + theme(axis.text.x = element_text(angle = 90, hjust = 1))
```

```{r}
assemblies = assemblies %>% mutate(Label=paste(Platform,Limit,Overlaps))
ggplot(assemblies %>% filter(ContigSize>100000),aes(x=Label, y=ContigSize, fill=Taxon)) + geom_bar(stat="identity") + coord_flip() + theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
